...
// Public Domain
// options --telnet=5501 --fdm=external
#include <errno.h>
#include <iostream>
#include <netdb.h>
...
try {
	...
	//f.write("set /sim/position/longitude-deg ");
	float h_above_ground_level=60.;//m
	f.write("set /position/altitude-agl-m %f",h_above_ground_level);
	f.write("get /position/altitude-agl-m");

	const char *p = f.read();
	if (p)
		std::cout << "RECV: " << p << std::endl;
	const char * aircraft_model=p;
	//f.write("set /sim/aircraft");
